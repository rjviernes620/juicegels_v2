...
function Le(e) {
  return Tt(ze(G - 1, Je(e === 91 ? e + 2 : e === 40 ? e + 1 : e)));
}
function pr(e) {
  for (; (A = U()) && A < 33; )
    W();
...
function ke(e, t, r, n, s, o, a, c, i) {
  for (var u = 0, l = 0, f = a, g = 0, $ = 0, m = 0, S = 1, P = 1, x = 1, h = 0, j = "", z = s, O = o, R = n, p = j; P; )
    switch (m = h, h = W()) {
      // (
      case 40:
        if (m != 108 && I(p, f - 1) == 58) {
      ...
      case 0:
        switch (h) {
          ...
          // { rule/at-rule
          default:
            if (ye(R = St(p, t, r, u, l, s, c, j, z = [], O = [], f, o), o), h === 123)
              if (l === 0)
                ke(p, t, R, R, z, o, f, c, O);
              else {
                switch (g) {
                  // c(ontainer)
                  case 99:
                ...
                l ? ke(e, R, R, n && ye(St(e, R, R, 0, 0, s, c, j, s, z = [], f, O), O), s, O, f, c, n ? z : O) : ke(p, R, R, R, [""], O, 0, c, O);
              }
        }
        u = l = $ = 0, S = x = 1, j = p = "", f = a;
        break;
...
function St(e, t, r, n, s, o, a, c, i, u, l, f) {
  for (var g = s - 1, $ = s === 0 ? o : [""], m = Mt($), S = 0, P = 0, x = 0; S < n; ++S)
    for (var h = 0, j = ee(e, g + 1, g = Et(P = a[S])), z = e; h < m; ++h)
      (z = Tt(P > 0 ? $[h] + " " + j : d(j, /&\f/g, $[h]))) && (i[x++] = z);
...
var Be, qe;
const L = typeof process < "u" && process.env !== void 0 && (process.env.REACT_APP_SC_ATTR || process.env.SC_ATTR) || "data-styled", Ft = "active", Ee = "data-styled-version", ue = "6.4.2", it = `/*!sc*/
...
const Lr = /* @__PURE__ */ Symbol.for("react.forward_ref");
function ht(e) {
...
const Jt = (e) => e == null || e === !1 || e === "", en = /* @__PURE__ */ Symbol.for("react.client.reference");
function Rt(e) {
  return e.$$typeof === en;
}
function Zt(e, t) {
  for (const r in e) {
    const n = e[r];
    e.hasOwnProperty(r) && !Jt(n) && (Array.isArray(n) && Ht.has(n) || he(n) ? t.push(It(r) + ":", n, ";") : xe(n) ? (t.push(r + " {"), Zt(n, t), t.push("}")) : t.push(It(r) + ": " + Qr(r, n) + ";"));
  }
}
function K(e, t, r, n, s = []) {
  ...
  return ht(e) ? (s.push(`.${e.styledComponentId}`), s) : Xr(e) ? (r ? (e.inject(r, n), s.push(e.getName(n))) : s.push(e), s) : Rt(e) ? s : xe(e) ? e.toString !== Object.prototype.toString ? (s.push(e.toString()), s) : (Zt(e, s), s) : (s.push(e.toString()), s);
}
const tn = Lt(ue);
class rn {
  constructor(t, r, n) {
...
function Xt({ options: e = le, plugins: t = ct } = le) {
  ...
  const i = Cr(a.concat($r((g) => c.push(g)))), u = (g, $ = "", m = "", S = "&") => {
    r = S, n = $, s = void 0;
    const P = (function(h) {
      const j = h.indexOf("//") !== -1, z = h.indexOf("}") !== -1;
      if (!j && !z) return h;
      if (!j) return Ve(h);
      const O = h.length;
...
function cn(e, t, r) {
  ...
  function $(S, P) {
    return (function(x, h, j) {
      const { attrs: z, componentStyle: O, defaultProps: R, foldedComponentIds: p, styledComponentId: y, target: J } = x, ne = C.useContext(te), T = mt(), de = x.shouldForwardProp || T.shouldForwardProp, k = at(h, ne, R) || le;
      let q, pe;
      {
        const Y = C.useRef(null), F = Y.current;
        if (F !== null && F[1] === k && F[2] === T.styleSheet && F[3] === T.stylis && F[7] === O && (function(se, E, v) {
          const _ = se, N = E;
          let ge = 0;
          for (const Z in N) if (Pt.call(N, Z) && (ge++, _[Z] !== N[Z])) return !1;
          return ge === v;
        })(F[0], h, F[4])) q = F[5], pe = F[6];
        else {
          q = (function(E, v, _) {
            const N = Object.assign(Object.assign({}, v), { className: void 0, theme: _ }), ge = E.length > 1;
            for (let Z = 0; Z < E.length; Z++) {
              const We = E[Z], Ie = he(We) ? We(ge ? Object.assign({}, N) : N) : We;
              for (const V in Ie) V === "className" ? N.className = Se(N.className, Ie[V]) : V === "style" ? N.style = Object.assign(Object.assign({}, N.style), Ie[V]) : V in v && v[V] === void 0 || (N[V] = Ie[V]);
            }
            return "className" in v && typeof v.className == "string" && (N.className = Se(N.className, v.className)), N;
          })(z, h, k), pe = (function(E, v, _, N) {
      ...
      const Ae = q.as || J, Fe = (function(Y, F, se, E) {
        const v = {};
        for (const _ in Y) Y[_] === void 0 || _[0] === "$" || _ === "as" || _ === "theme" && Y.theme === se || (_ === "forwardedAs" ? v.as = Y.forwardedAs : E && !E(_, F) || (v[_] = Y[_]));
        return v;
      })(q, Ae, k, de);
      let De = Se(p, y);
      return pe && (De += " " + pe), q.className && (De += " " + q.className), Fe[Qe(Ae) && Ae.includes("-") ? "class" : "className"] = De, j && (Fe.ref = j), sr(Ae, Fe);
    })(m, S, P);
  }
  $.displayName = i;
  let m = C.forwardRef($);
